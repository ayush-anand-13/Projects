class GamesController < ApplicationController
	def new
		if not session[:user_id].nil?
			...
			puts @user.games.where(category: "Entertainment", subcat: "Books").first.nil?
			render 'new'
		else
	...
	def quest
		@user = User.find(session[:user_id])
		start = @user.games.where(category: request[:id1], subcat: request[:id2]).first.state.to_i
